  ...
  members.forEach((m) => {
    ...
    txns.savings?.forEach((t) => {
      rows.push({
        ...
        Amount: Number(t.amount), Owed: "", Note: t.note || "",
      });
    });

    // Loan rows: sort chronologically first so the running "Owed" balance is accurate,
    // matching computeBalances() in ChamaLedger.jsx (repayment decreases, everything else increases).
    const loanTxns = (txns.loans || [])
      .slice()
    ...
    loanTxns.forEach((t) => {
      ...
      const isDecrease = t.type === "repayment";
      owed += isDecrease ? -amt : amt;
      rows.push({
        Month: t.date.slice(0, 7), Date: t.date, Member: m.name,
        Account: "Loans", Type: LOAN_LABELS[t.type] || t.type,
        Amount: isDecrease ? -amt : amt, Owed: owed, Note: t.note || "",
      });
    });
  });
...
export function exportMonthlyReportToExcel(state, chamaName = "Chama") {
  ...
  state.members.forEach((m) => {
    ...
    const rows = detailRowsFor(state, m.id)
      .slice()
      .reverse()
      .map((r) => ({
        Date: r.Date,
