 ...
 *
 * Account type classification (standard accounting):
 * - Cash/Bank              -> Asset
 * - Member's Loan Account  -> Asset (money owed TO the group)
 * - Member's Savings       -> Liability (money owed BY the group to the member)
...
export function getAccountType(accountName) {
  if (ACCOUNT_TYPES[accountName]) return ACCOUNT_TYPES[accountName];
  if (accountName.includes("Loan Account")) return "Asset";
  if (accountName.includes("Savings")) return "Liability";
  return "—";
}
...
export function getDoubleEntry(account, type, memberName) {
  ...
  const entry = map[account]?.[type] || { debit: "—", credit: "—", note: "" };
  return {
    ...entry,
    debitType: getAccountType(entry.debit),
    creditType: getAccountType(entry.credit),
  };
}
