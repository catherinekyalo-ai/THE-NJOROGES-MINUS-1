...
export default function MonthlySummary({ state, C }) {
  const summary = useMemo(() => buildMonthlySummary(state), [state]);
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.rule}`, borderRadius: 10, padding: "18px 20px", marginBottom: 28 }}>
      ...
      {months.length === 0 ? (
        <div style={{ fontSize: 13, color: C.textSoft, fontFamily: "'Inter', sans-serif" }}>No transactions recorded yet.</div>
      ) : (
